...
export class ObservableSet<T> extends ObservableVariable<ReadonlySet<T>> {
    ...
    protected _onAdd: Set<(value: T, oSet: ObservableSet<T>) => void> = new Set();
    protected _onDelete: Set<(value: T, oSet: ObservableSet<T>) => void> = new Set();
    ...
    constructor(value: ObservableSet<T> | Set<T> | T[], options?: IObservableVariableOptions) {
     ...
     * 当改变变量值的时候触发
     */
    on(event: 'set', callback: (newValue: Set<T>, oldValue: ReadonlySet<T>, oSet: this) => void): void;
    /**
     * 在变量值发生改变之前触发，返回一个新的值用于替换要设置的值
     * 注意：该回调只允许设置一个，重复设置将覆盖之前的回调。该回调不会受ensureChange的影响，只要用户设置值就会被触发
     */
    on(event: 'beforeSet', callback: (newValue: Set<T>, oldValue: ReadonlySet<T>, oSet: this) => ReadonlySet<T>): void;
    /**
     * 当向集合中添加元素时触发
     */
    ...
    once(event: 'set', callback: (newValue: Set<T>, oldValue: ReadonlySet<T>, oSet: this) => void): void;
    once(event: 'beforeSet', callback: (newValue: Set<T>, oldValue: ReadonlySet<T>, oSet: this) => ReadonlySet<T>): void;
    ...
    off(event: 'set', callback?: (newValue: Set<T>, oldValue: ReadonlySet<T>, oSet: this) => void): void;
    off(event: 'beforeSet', callback?: (newValue: Set<T>, oldValue: ReadonlySet<T>, oSet: this) => ReadonlySet<T>): void;
    ...
    /**
     * 返回一个包含[value, value]键值对的Iterator对象，返回的迭代器的迭代顺序与Set对象的插入顺序相同
     */
     ...
     */
    forEach<H = undefined>(callback: (this: H, value: T, value2: T, set: ReadonlySet<T>) => void, thisArg?: H): void {
        this._value.forEach(callback, thisArg);
    }

    /**
     * 返回一个布尔值来指示对应的值value是否存于Set中
     */
    has(value: T): boolean {
...
export function oSet<T>(value: ObservableSet<T> | Set<T> | T[], options?: IObservableVariableOptions): ObservableSet<T> {
